...
def p2img_forward(image, target, scale, patch_size, stride, model):
    ...
    un_hr = [
        un_lr[i] * scale - overlap * scale if i > 4 else un_lr[i]
        for i in range(len(un_lr))
    ]
    ...
    # discard of overlap
    output = output[:, :, overlap:-overlap, overlap:-overlap]
...
def plot_out_target(output, target):
    ...
    print(output_permute.shape, target_permute.shape)
